...
function Settings({ settings, setSettings, portfolio, stockAssets, reitAssets }) {
  ...
  useEffect(() => {
    ...
    // Verificar se os totais ultrapassam os limites
    setValidationErrors({
      stocks: stocksTotal > 100,
      reits: reitsTotal > 100
    });
    
    // Auto-ajustar para o caso de apenas um ativo
  ...
  const handleSaveSettings = () => {
    ...
    if (stocksAllocationTotal > 100 || reitsAllocationTotal > 100) {
      alert('A soma dos percentuais ideais não pode ultrapassar 100%. Por favor, ajuste as alocações antes de salvar.');
      return;
    }
    
    // Verificar se a distribuição entre ações e FIIs está alinhada com as alocações individuais
    if (stockAssets.length > 0 && stocksAllocationTotal > 0 && 
        Math.abs(settings.stocksPercentage - stocksAllocationTotal) > 0.1) {
      if (!window.confirm(`A distribuição geral de ações (${settings.stocksPercentage.toFixed(1)}%) não corresponde ao total das alocações individuais (${stocksAllocationTotal.toFixed(1)}%). Deseja continuar mesmo assim?`)) {
        return;
      }
    }
    
    if (reitAssets.length > 0 && reitsAllocationTotal > 0 && 
        Math.abs(settings.reitsPercentage - reitsAllocationTotal) > 0.1) {
      if (!window.confirm(`A distribuição geral de FIIs (${settings.reitsPercentage.toFixed(1)}%) não corresponde ao total das alocações individuais (${reitsAllocationTotal.toFixed(1)}%). Deseja continuar mesmo assim?`)) {
        return;
      }
    }
    ...
    <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6 mt-6">
        ...
        <Card>
          <div className="flex justify-between items-center mb-4">
            <h2 className="text-lg font-bold">Alocação de Ações</h2>
          ...
          {stockAssets.length === 1 && (
            <div className="mt-3 text-sm text-blue-600">
              <p>Como há apenas uma ação, o percentual ideal foi definido automaticamente para 100%.</p>
            </div>
          )}
        </Card>
        ...
        <Card>
          <div className="flex justify-between items-center mb-4">
            <h2 className="text-lg font-bold">Alocação de FIIs</h2>
          ...
          {reitAssets.length === 1 && (
            <div className="mt-3 text-sm text-green-600">
              <p>Como há apenas um FII, o percentual ideal foi definido automaticamente para 100%.</p>
            </div>
          )}
        </Card>
      </div>
      ...
      <div className="bg-blue-50 border border-blue-200 rounded-md p-4 mb-6">
        ...
        <ul className="list-disc pl-5 text-sm text-blue-700 space-y-1">
          <li>A soma dos percentuais ideais para cada tipo de ativo (ações ou FIIs) não deve ultrapassar 100%.</li>
          <li>Quando há apenas um ativo de cada tipo, seu percentual é automaticamente definido como 100%.</li>
          <li>Os percentuais são usados para sugerir compras na tela de Investir, priorizando os ativos mais distantes da alocação ideal.</li>
        </ul>
      </div>
