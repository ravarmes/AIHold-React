...
function Invest({ 
  ...
  const generateSuggestions = () => {
    ...
    if (stocksAllocation > 0) {
      const stocksWithTarget = stockAssets.map(asset => {
        // Calcular o percentual ideal de cada ativo na carteira total
        // Conversão do percentual interno para o percentual na carteira
        const internalPct = settings.assetAllocations[asset.ticker] || 0;
        const targetPct = (internalPct / 100) * settings.stocksPercentage;
    ...
    if (reitsAllocation > 0) {
      const reitsWithTarget = reitAssets.map(asset => {
        ...
        // Conversão do percentual interno para o percentual na carteira
        const internalPct = settings.assetAllocations[asset.ticker] || 0;
        const targetPct = (internalPct / 100) * settings.reitsPercentage;
        const currentPct = asset.percentage || 0;
        const deficitPct = targetPct - currentPct;
  ...
  // Função para executar a compra e atualizar a carteira
  const executeInvestment = () => {
    // Verificar se há sugestões selecionadas e se o valor não ultrapassa o disponível
    if (selectedTickers.length === 0 || remainingValue < 0) {
      return;
    }

    // Filtrar apenas as sugestões selecionadas
    const selectedSuggestions = investSuggestions.filter(
      suggestion => selectedTickers.includes(suggestion.ticker)
    );

    // Criar cópia da carteira atual
    const updatedPortfolio = [...portfolio];

    // Para cada sugestão selecionada, atualizar a carteira
    selectedSuggestions.forEach(suggestion => {
      const existingAssetIndex = updatedPortfolio.findIndex(
        asset => asset.ticker === suggestion.ticker
      );

      if (existingAssetIndex >= 0) {
        // Ativo já existe na carteira, apenas aumentar a quantidade
        updatedPortfolio[existingAssetIndex].quantity += suggestion.quantityToBuy;
      } else {
        // Ativo novo, adicionar à carteira
    ...
    // Atualizar a carteira no estado e no localStorage
    setPortfolio(updatedPortfolio);
    localStorage.setItem('aiholdPortfolio', JSON.stringify(updatedPortfolio));

    // Limpar as sugestões e seleções
    setInvestSuggestions([]);
    setSelectedTickers([]);

    // Opcional: redefinir o valor de investimento
    // setInvestmentValue(0);

    // Mostrar mensagem de sucesso
    alert('Investimento realizado com sucesso! A carteira foi atualizada.');
  };
